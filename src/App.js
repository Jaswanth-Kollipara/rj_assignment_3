...
class App extends Component {
  ...
  tick = () => {
    ...
    if (time > 1) {
      ...
      this.setState({isPlaying: false}, this.clear)
    }
  }
  ...
  clickImg = id => {
    ...
    const randId = Math.floor(Math.random() * imagesList.length)
    const updatedImg = imagesList[randId]
    if (id === currentImg.id) {
      this.setState(prevState => ({
        count: prevState.count + 1,
        currentImg: updatedImg,
      }))
    } else {
      this.setState({isPlaying: false}, this.clear)
    }
  ...
  render() {
    const {currentImg, currentTab, count, time, isPlaying} = this.state
    const updatedList = imagesList.filter(item => item.category === currentTab)
    ...
    return (
      <>
        ...
        {isPlaying && (
          <div>
            <img src={currentImg.imageUrl} alt="match" />
            <ul>
              {tabsList.map(item => (
                <Tabs data={item} key={item.id} changeTab={this.changeTab} />
              ))}
            </ul>
            <ul>
              {updatedList.map(item => (
                <Images data={item} key={item.id} clickImg={this.clickImg} />
              ))}
            </ul>
          </div>
        )}
